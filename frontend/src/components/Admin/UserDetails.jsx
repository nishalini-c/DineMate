...
const UserDetails = () => {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        const response = await axios.get('http://localhost:9000/api/users/users', { withCredentials: true });
  ...
  const handleSubmit = async () => {
    try {
      await axios.put(`http://localhost:9000/api/users/${editUser._id}`, editUser);
      const response = await axios.get('http://localhost:9000/api/users/users', { withCredentials: true });
      setUsers(response.data);
      setEditUser(null);
      setShowModal(false);

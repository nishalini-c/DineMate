...
const View = () => {
  ...
  useEffect(() => {
    const fetchTableReservations = async () => {
      try {
        const response = await axios.get('http://localhost:9000/api/table/');
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`http://localhost:9000/api/table/${id}`);
  ...
  const handleSubmit = async () => {
    try {
      await axios.put(
        `http://localhost:9000/api/table/${editReservation._id}`,
        editReservation
      );
      setShowModal(false);
